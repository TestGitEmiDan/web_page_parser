...
public:
  ...
  inline static std::string current_time_str(const char* format = "%Y_%m_%d_%H_%M_%S");
};
...
inline std::string logger::current_time_str(const char* format)
{
  const auto current_time_t = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
  std::stringstream ss;
  ss << std::put_time(std::localtime(&current_time_t), format);
